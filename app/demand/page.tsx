...
export default function DemandPage() {
  ...
  const totalDemand = rawCities.reduce((s: number, c: any) => s + c.totalDemand, 0)
  const total3MR = rawCities.reduce((s: number, c: any) => s + c.demand3MR, 0)
  const avgDel = rawCities.length ? rawCities.reduce((s: number, c: any) => s + c.delPct3MR, 0) / rawCities.length : 0
  const uptickCities = rawCities.filter((c: any) => c.trendDirection === 'up').length
  ...
  const cityGTDelPct = cityGT.demand3MR > 0 ? (cityGT.delivered3MR / cityGT.demand3MR) * 100 : avgDel
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCard label="3MR Demand" value={formatNumber(total3MR)} accent="blue" />
        <StatCard label="Avg 3MR DEL%" value={formatPct(avgDel)} accent={avgDel >= 80 ? 'green' : avgDel >= 60 ? 'amber' : 'red'} />
        <StatCard label="Cities Trending ▲" value={`${uptickCities} / ${rawCities.length}`} accent="green" />
      </div>
        ...
        <>
          <div className="flex flex-wrap items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3">
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors flex items-center gap-1.5 ${primeOnly ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-600 border-slate-200 hover:border-blue-400'}`}
            >
              <Star className="w-3 h-3" />Prime Only
            </button>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {cities.map((city: any) => (
                    <>
                      <tr key={city.city} className="bg-slate-50 hover:bg-slate-100/80 cursor-pointer font-medium transition-colors" onClick={() => city.hubs?.length > 0 && toggleCity(city.city)}>
                        ...
                        <td className="text-right px-3 py-3 text-blue-600 font-semibold">{formatNumber(city.demand3MR)}</td>
                        <td className="text-right px-3 py-3"><DelPctCell value={city.delPct3MR} /></td>
                        <td className="px-3 py-3"><TrendBadge direction={city.trendDirection as TrendDirection} pct={city.trendPct} /></td>
                      ...
                      {expandedCities.has(city.city) && city.hubs?.map((hub: any) => (
                        <tr key={hub.hub} className="bg-white hover:bg-blue-50/30 transition-colors">
                          ...
                          <td className="text-right px-3 py-2.5 text-slate-600">{formatNumber(hub.demand3MR)}</td>
                          <td className="text-right px-3 py-2.5 text-blue-500">{formatNumber(hub.demand3MR)}</td>
                          <td className="text-right px-3 py-2.5"><DelPctCell value={hub.delPct3MR} /></td>
                          <td className="px-3 py-2.5"><TrendBadge direction={hub.trendDirection as TrendDirection} pct={hub.trendPct} /></td>
                  ...
                  <tr className="bg-slate-100 border-t-2 border-slate-300 font-semibold">
                    ...
                    <td className="text-right px-3 py-3 font-mono font-bold text-blue-700">{formatNumber(cityGT.demand3MR)}</td>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-3 py-3 font-medium text-slate-600">Prime</th>
                  ...
                  <th className="px-4 py-3 font-medium text-slate-600">vs D-1</th>
                </tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {clients.map((client: any) => (
                    <tr key={client.clientName} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-3 py-3">{client.isPrime ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200"><Star className="w-3 h-3" />Prime</span> : <span className="text-slate-400 text-xs">—</span>}</td>
                      <td className="text-right px-3 py-3 tabular-nums text-slate-700">{formatNumber(client.totalAWBs)}</td>
                      <td className="text-right px-3 py-3 tabular-nums text-blue-600">{formatNumber(client.awbs3MR)}</td>
                      ...
                      <td className="px-4 py-3"><TrendBadge direction={client.trendDirection as TrendDirection} pct={client.trendPct} /></td>
                    </tr>
                  ))}
                  {/* Client grand total */}
                  <tr className="bg-slate-100 border-t-2 border-slate-300 font-semibold">
                    ...
                    <td className="text-right px-3 py-3 font-mono font-bold text-blue-700">{formatNumber(clientGT.awbs3MR)}</td>
