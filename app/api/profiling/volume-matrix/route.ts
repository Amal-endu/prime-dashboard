...
rider_summary AS (
  ...
  FROM classified c LEFT JOIN hub_mapping hm ON LOWER(c.hub) = LOWER(hm.hub)
    ...
    let where = 'WHERE 1=1'
    let paramIdx = 6
    if (city) { where += ` AND COALESCE(vrs.city, 'Unmapped') = $${paramIdx++}`; filterParams.push(city) }
    if (hub)  { where += ` AND vrs.hub = $${paramIdx++}`;                        filterParams.push(hub) }
  ...
  JOIN rider_daily AS rd ON rd.rider_id = vrs.rider_id
    AND rd.date BETWEEN (vrs.max_date - ((SELECT window_days FROM cfg) - 1) * INTERVAL '1 day')::DATE
                    AND vrs.max_date
  ${where}
