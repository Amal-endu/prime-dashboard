...
cfg AS (
  SELECT
    ?::INTEGER AS window_days, ?::INTEGER AS new_rider_days,
    ?::DOUBLE AS evening_threshold, ?::DOUBLE AS cross_threshold, ?::DOUBLE AS regular_threshold
),
rider_window AS (
...
agg AS (
  SELECT rider_id, MAX(rider_name) AS rider_name, MAX(hub) AS hub,
    ...
    SUM(had_any_login) AS login_days, SUM(had_morning_login) AS morning_login_days,
...
rider_summary AS (
  ...
  FROM classified c LEFT JOIN hub_mapping hm ON LOWER(c.hub) = LOWER(hm.hub)
    ...
    let where = 'WHERE 1=1'
    if (city) { where += " AND COALESCE(vrs.city, 'Unmapped') = ?"; filterParams.push(city) }
    if (hub)  { where += ' AND vrs.hub = ?';                         filterParams.push(hub) }

    // Sum attempted_total from rider_daily over the same 30-day window used by the inline CTE.
    // We join on rider_id so each rider's volume is attributed to their regularity/behaviour tag.
    const matrixRows = await query<{
      regularity_tag: string
      login_behaviour_tag: string
      total_attempts: number
    }>(
      `WITH ${classifyCte},
  date_range AS (
    SELECT (max_date - to_days(29)) AS start_date, max_date FROM v_max_date
  )
  SELECT
    vrs.regularity_tag,
    vrs.login_behaviour_tag,
    COALESCE(SUM(rd.attempted_total), 0) AS total_attempts
  FROM rider_summary AS vrs
  JOIN rider_daily AS rd
    ON rd.rider_id = vrs.rider_id
   AND rd.date BETWEEN (SELECT start_date FROM date_range) AND (SELECT max_date FROM date_range)
  ${where}
  GROUP BY vrs.regularity_tag, vrs.login_behaviour_tag
  ORDER BY vrs.regularity_tag, vrs.login_behaviour_tag`,
      [...cfgParams, ...filterParams],
    )

    const [{ grand_total }] = await query<{ grand_total: number }>(
      `WITH ${classifyCte},
  date_range AS (
    SELECT (max_date - to_days(29)) AS start_date, max_date FROM v_max_date
  )
  SELECT COALESCE(SUM(rd.attempted_total), 0) AS grand_total
  FROM rider_summary AS vrs
  JOIN rider_daily AS rd
    ON rd.rider_id = vrs.rider_id
   AND rd.date BETWEEN (SELECT start_date FROM date_range) AND (SELECT max_date FROM date_range)
  ${where}`,
    ...
    for (const r of matrixRows) {
      ...
      const n   = Number(r.total_attempts)
    ...
    console.error('[API /profiling/volume-matrix]', err)
